...
export const SetupScreen: React.FC<SetupScreenProps> = ({ teams, setTeams, onStart }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 flex flex-col h-full overflow-y-auto">
      ...
      <div className="mt-8 pt-6 border-t border-slate-800 flex justify-center pb-8">
        <button
          onClick={onStart}
      ...
      {teams.length < 2 && teams.length > 0 && (
         <p className="text-center text-orange-400/80 mt-2 text-sm font-medium">Je hebt minimaal 2 teams nodig om te starten.</p>
      )}
    </div>
  );
};